...
class Inventory
{
	...
	public function getPtNames()
	{
		$sqlQuery = "SELECT supplier_name  FROM " . $this->supplierTable;
		return $this->getData($sqlQuery);
	}

	public function getProductNames()
	{
		$sqlQuery = "SELECT pname  FROM " . $this->productTable;
		return $this->getData($sqlQuery);
	}
	public function getPurchaseNames()
	{
		$sqlQuery = "SELECT pid FROM " . $this->productTable;
		return $this->getData($sqlQuery);
	}
	...
	{
		$sqlQuery = "SELECT pid, c.name, bname, pname, p.model, p.quantity, supplier_name, p.status
	...
	public function getProductListInactive()
	{
		
		$sqlQuery = "SELECT pid,p.pname, p.quantity, s.supplier_name, p.status 
			FROM " . $this->productTable . " as p 
			INNER JOIN " . $this->supplierTable . " as s ON s.supplier_id = p.supplier
			WHERE p.status = 'inactive'";
	
		if (isset($_POST["search"]["value"])) {
			$sqlQuery .= ' AND (p.pname LIKE "%' . $_POST["search"]["value"] . '%" ';
			$sqlQuery .= ' OR s.supplier_name LIKE "%' . $_POST["search"]["value"] . '%")';
		...
		if (isset($_POST['order'])) {
			...
			$sqlQuery .= ' ORDER BY p.pname DESC ';
		}
		if ($_POST['length'] != -1) {
			$sqlQuery .= ' LIMIT ' . $_POST['start'] . ', ' . $_POST['length'];
		}
	...
	{
		$sqlInsert = "
			INSERT INTO " . $this->productTable . "(categoryid, brandid, pname, model, description, quantity,  supplier) 
			VALUES ('" . $_POST["categoryid"] . "', '" . $_POST['brandid'] . "', '" . $_POST['pname'] . "', '" . $_POST['pmodel'] . "', '" . $_POST['description'] . "', '" . $_POST['quantity'] . "', '" . $_POST['supplierid'] . "')";
		mysqli_query($this->dbConnect, $sqlInsert);
	...
	public function updateProduct()
	{
		if ($_POST['pid']) {
			$sqlUpdate = "UPDATE " . $this->productTable . " 
				SET categoryid = '" . $_POST['categoryid'] . "', brandid='" . $_POST['brandid'] . "', pname='" . $_POST['pname'] . "', model='" . $_POST['pmodel'] . "', description='" . $_POST['description'] . "', quantity='" . $_POST['quantity'] .  "', supplier='" . $_POST['supplierid'] . "' WHERE pid = '" . $_POST["pid"] . "'";
			mysqli_query($this->dbConnect, $sqlUpdate);
			echo 'Product Update';
		}
	}
...
{
	$sqlQuery = "SELECT p.pid, p.pname, p.model, p.quantity as product_quantity, s.quantity as recieved_quantity, (p.quantity - s.quantity) as inventory_in_hand
		FROM " . $this->productTable . " as p
	...
	while ($inventory = mysqli_fetch_assoc($result)) {
		...
		$inventory['inventory_in_hand'] = $inventory['product_quantity'] - $inventory['recieved_quantity'];
		...
		$inventoryRow[] = $inventory['inventory_in_hand'];
